...
vector<string> glob(const vector<string> &patterns) {
  ...
  glob(patterns[0].c_str(), 0, nullptr, &glob_results);

  auto iter = patterns.begin();

  while (++iter != patterns.end()) {
    glob(iter->c_str(), GLOB_APPEND, nullptr, &glob_results);
  }

  for (unsigned path_index = 0; path_index < glob_results.gl_pathc;
       path_index++) {
...
string format(const char *format, ...) {
  ...
  while (!done) {
    ...
    } else if ((unsigned)return_value >= buffer_size_bytes) {
